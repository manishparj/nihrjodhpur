...
//echo '<script type="text/javascript">alert("'.$nma.'")</script>';
	...
	$sql1="UPDATE info_en SET title=(:var1), type=(:var2), upl_date=(:var3), status=(:var4), last_dt=(:var5), category=NULLIF('$category', '') WHERE id=$edid";
...
if(move_uploaded_file($file_loc,$folder.$final_file) )
 ...
 $sql3="UPDATE doc_en SET doc_id=$edid,doc_main=(:doc_file) where doc_id=$edid";
...
// pdf1
	...
	{
$file1 = "doc".time().$_FILES['uplaz']['name'];
...
if(move_uploaded_file($file_loc1,$folder.$final_file1) )
 ...
 //process your file and data
 $sql31="UPDATE doc_en SET doc_id=$edid,doc_nm_a=NULLIF('$nma', ''),doc_a=NULLIF('$doc_a', '') where doc_id=$edid";
...
// pdf2
	...
	{
$file2 = "doc".time().$_FILES['uplbz']['name'];
...
if(move_uploaded_file($file_loc2,$folder.$final_file2) )
 ...
 //process your file and data
 $sql32="UPDATE doc_en SET doc_id=$edid,doc_nm_b=NULLIF('$nmb', ''),doc_b=NULLIF('$doc_b', '') where doc_id=$edid";
...
if(move_uploaded_file($file_loc3,$folder.$final_file3) )
 ...
 //process your file and data
 $sql33="UPDATE doc_en SET doc_id=$edid,doc_nm_c=NULLIF('$nmc', ''),doc_c=NULLIF('$doc_c', '') where doc_id=$edid";
...
if(move_uploaded_file($file_loc4,$folder.$final_file4) )
 ...
 //process your file and data
 $sql34="UPDATE doc_en SET doc_id=$edid,doc_nm_d=NULLIF('$nmd', ''),doc_d=NULLIF('$doc_d', '') where doc_id=$edid";
...
if(move_uploaded_file($file_loc5,$folder.$final_file5) )
 ...
 //process your file and data
 $sql35="UPDATE doc_en SET doc_id=$edid,doc_nm_e=NULLIF('$nme', ''),doc_e=NULLIF('$doc_e', '') where doc_id=$edid";
...
if(move_uploaded_file($file_loc6,$folder.$final_file6) )
 ...
 //process your file and data
 $sql36="UPDATE doc_en SET doc_id=$edid,doc_nm_f=NULLIF('$nmf', ''),doc_f=NULLIF('$doc_f', '') where doc_id=$edid";
...
if(move_uploaded_file($file_loc7,$folder.$final_file7) )
 ...
 //process your file and data
 $sql37="UPDATE doc_en SET doc_id=$edid,doc_nm_g=NULLIF('$nmg', ''),doc_g=NULLIF('$doc_g', '') where doc_id=$edid";
...
if(move_uploaded_file($file_loc8,$folder.$final_file8) )
 ...
 //process your file and data
 $sql38="UPDATE doc_en SET doc_id=$edid,doc_nm_h=NULLIF('$nmh', ''),doc_h=NULLIF('$doc_h', '') where doc_id=$edid";
...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        ...
                        <a href="insert.php" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
                                class="fas fa-down fa-sm text-white-50"></i> Insert new <?php echo $name; ?></a>
                    </div>
                                            ...
                                            <?php
																						...
																						foreach($results9 as $result19)
																						{
																							...
																							$subjectOutput = array_filter($document);
																								?>
                                            <td class="text-nowrap"><i class="fa fa-angle-right" aria-hidden="true"
                                                    style="color:blue"></i>&nbsp;<a
                                                    href="en_doc/<?php echo htmlentities($result19->doc_main);?>"
                                                    target="_blank">Document <i class="fa fa-download"
                                                        aria-hidden="true"></i></a><br>
                                                <?php
																						$length = count($strOutput);

																								for ($i = 0; $i < $length; $i++) {
																									echo '<i class="fa fa-angle-right" aria-hidden="true" style="color:blue"></i>&nbsp;<a href="en_doc/'.$subjectOutput[$i].'">'.$strOutput[$i].'&nbsp;<i class="fa fa-download" aria-hidden="true"></i></a><br>';
																									 // echo '<script type="text/javascript">alert("'.$strOutput[$i].'")</script>';
																								}
																						 ?>
                                                <?php }} ?>
                                            <td>
                                                <a class="modalLink" href="#myModal" data-toggle="modal"
                                                    data-target="#myModal"
                                                    data-id="<?php echo htmlentities($result19->doc_id); ?>"
                                                    data-name="<?php echo htmlentities($result->type); ?>"
                                                    data-addr="<?php echo htmlentities($result19->doc_main); ?>">
                                                    &nbsp; <i class="fa fa-edit"></i></a>&nbsp;&nbsp;
                                                </a>
                                                <a href="tender.php?del=<?php echo $result->id;?>&dcid=<?php echo htmlentities($result19->doc_id);?>&name=<?php echo htmlentities($name);?>"
    ...
    $('.modalLink').click(function() {
        ...
        $.ajax({
            url: "edit2.php?famID=" + famID + "&famAddr=" + famAddr,
            cache: false,
