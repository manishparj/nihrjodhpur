...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql1 = "UPDATE info_hi SET title=(:var1), type=(:var2), upl_date=(:var3), status=NULLIF('$status', ''), last_dt=NULLIF('$lastdt', ''),category=NULLIF('$category', '') WHERE id=$edid";
        ...
        if (!empty($_FILES['uplmdoc']['name'])) //new image uploaded
        {
            ...
            $sql3 = "UPDATE doc_hi SET doc_id=$edid,doc_main=(:doc_file),doc_apform=NULLIF('$doc_apform', '') where doc_id=$edid";
                                                    ...
                                                    <tr>
                                                        ...
                                                        if ($query9->rowCount() > 0) {
                                                            foreach ($results9 as $result19) {                ?>
                                                                <td><a href="hi_doc/<?php echo htmlentities($result19->doc_main); ?>" target="_blank"><i class="fa fa-download" aria-hidden="true"></i></a></td>
                                                        <?php }
                                                        } ?>
                                                        <td class="text-nowrap">
                                                            ...
                                                            <a href="viewh.php?del=<?php echo $result->id; ?>&dcid=<?php echo htmlentities($result19->doc_id); ?>&name=<?php echo htmlentities($name); ?>" onclick="return confirm('Do you want to Delete');"><i class="fa fa-trash" style="color:red"></i></a>&nbsp;&nbsp;
                                                        </td>
                                                    </tr>
