...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql1 = "UPDATE emp_details SET emp_name=(:var1), emp_name_hi=(:var3), emp_desig=(:var2), emp_desig_hi=(:var4), emp_contact=NULLIF('$emp_tel', ''), emp_email=NULLIF('$emp_email',''), research_interest=NULLIF('$research_interest',''), emp_status=NULLIF('$emp_status', ''), emp_seniority=NULLIF('$emp_sr_rank', '') WHERE emp_id=$edid";
        $query1 = $dbh->prepare($sql1);
        $query1->bindParam(':var1', $emp_name, PDO::PARAM_STR);
        $query1->bindParam(':var2', $emp_desig, PDO::PARAM_STR);
        $query1->bindParam(':var3', $emp_name_hi, PDO::PARAM_STR);
        $query1->bindParam(':var4', $emp_desig_hi, PDO::PARAM_STR);
                                            ...
                                            if ($query->rowCount() > 0) {
                                                foreach ($results as $result) {
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><?php echo htmlentities($result->emp_status); ?></td>
                                                        <td><?php echo htmlentities($result->emp_seniority); ?></td>
