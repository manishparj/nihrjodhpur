...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $emp_name_hi = $_POST['emp_name_hi'];

        $emp_desig = $_POST['emp_desig'];
        $emp_desig_hi = $_POST['emp_desig_hi'];
    ...
    $googlescholar = $_POST['googlescholar'];
        ...
        $sql1 = "UPDATE emp_details SET emp_contact=NULLIF('$emp_tel', ''), emp_email=NULLIF('$emp_email',''), research_interest=NULLIF('$research_interest',''), googlescholar=NULLIF('$googlescholar','') WHERE emp_id=$edid";
    ...
    if (isset($_POST['submit1'])) {
        ...
        $comp_project = $_POST['comp_project'];
        $ong_project = $_POST['ong_project'];



        $sql2 = "UPDATE emp_profile SET brief_intro=NULLIF('$brief_intro', ''), academic=NULLIF('$academic',''), publication=NULLIF('$publication',''),comp_project=NULLIF('$comp_project',''),ong_project=NULLIF('$ong_project','')  WHERE emp_id=$edid";
        $query2 = $dbh->prepare($sql2);
        $query2->execute();
                                ...
                                <div class="table-responsive">
                                    ...
                                    <table id="dataTable" class="table table-bordered table-hover" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
                                            ...
                                            $sql = "SELECT * from emp_details where emp_name = (:type)";
                                            $query = $dbh->prepare($sql);
                                            ...
                                            if ($query->rowCount() > 0) {
                                                foreach ($results as $result) {   ?>
                                                        ...
                                                        <td><img src="img/our_team/<?php echo "scientist/".htmlentities($result->emp_image);?>" style="width:50px;" /></td>
                                                        ...
                                                        <td><a class="modalLink1" href="#myModal1" data-toggle="modal" data-target="#myModal1" data-id="<?php echo htmlentities($result->emp_id); ?>" data-name="<?php echo htmlentities($result->emp_type); ?>"><i class="fa fa-edit"></i></a></td>
                                                        <td><?php echo htmlentities($result->googlescholar); ?></td>
